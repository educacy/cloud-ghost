...
function AnalyticsCard01() {
  ...
  let [violetLineDataX, setVioletLineDataX] = React.useState([]);
  let [grayLineDataX, setGrayLineDataX] = React.useState([]);
  let violetLineData = [];
  let grayLineData = [];

  React.useEffect(()=> {
    const fetchData= async()=> {
        const url = `${Config.ApiDomain}/query/aws/compute-instance-metric-hourly`
        const labelSet = []
        const dataSet1 = [];
        const dataSet2 = [];
      await axios.get(url).then((data)=> {
         ...
         for (const val of res) {
             ...
             dataSet1.push(val.average);
             dataSet2.push(val.minimum)
             ind+=1;
         }
        ...
        //  console.log(dataSet1,dataSet)
         ...
         setData({
             ...
             datasets: [
               {
                 label: 'Dataset ID',
                 data:[0.34655555555555556, 0.37458330836194437, 0.3514444629663888, 0.3408055625030015, 0.5310277777777777, 0.40524999999999994, 0.42891663657827933],
